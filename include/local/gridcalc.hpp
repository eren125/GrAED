...
double &temperature, double &cutoff, std::string &element_guest_str, 
double &approx_spacing, double &energy_threshold, double &access_coeff, double &molar_mass, 
double &boltzmann_energy_lj, double &boltzmann_energy_sq_lj, double &boltzmann_energy_3_lj, 
double &boltzmann_energy_4_lj, double &mean_grid, double &std_grid, double &sum_exp_energy, 
gemmi::Grid<double> &grid, bool calc_block=true) {
  /**
  ...
  for (auto site: all_sites) {
    ...
    grid.use_points_around<true>(site.fract, access_coeff*sigma, [&](double& ref, double d2){
      if (calc_block){
        ...
        ref += 4*R*energy_lj_opt(epsilon,sigma_6,inv_distance_6,inv_cutoff_6,inv_distance_12,inv_cutoff_12);
      }
      else {ref = MAX_ENERGY;}
    }, false);
